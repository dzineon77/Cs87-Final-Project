...
CUDAKernel::CUDAKernel(int argc, char *argv[])
{
    ...
    // dim3 blocks_value((game_board_width_CPU / BLOCK_SIZE), (game_board_height_CPU / BLOCK_SIZE), 1);
    // dim3 threads_block_value(BLOCK_SIZE, BLOCK_SIZE, 1);

    // blocks = blocks_value;
    // threads_block = threads_block_value;
